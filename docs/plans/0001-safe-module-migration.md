...
3. **DONE: `safe` と対応 operation を policy spec と test vector に定義する。** `account` は Safe address（`account == safe`）を意味し、Safe operation は empty calldata + `Enum.Operation.Call` に固定。既存の 8 public input 順序と hex は維持（vector version 2 はメタデータ追加のみ）。module 実装は含まない。
