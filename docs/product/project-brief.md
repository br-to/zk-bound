...
An owner can delegate narrowly scoped wallet capabilities to an AI agent without
...
- verification happens at the execution boundary; and
...
- wallet and smart-account developers adding constrained agent permissions;
...
- model a wallet action and an owner-defined permission policy;
- bind an action, policy commitment, account, chain, and replay protection into
  a proof statement;
- verify the proof at or immediately before the wallet execution boundary;
...
2. An allowed action executes in a local or test environment.
...
- Which first wallet action creates the clearest demonstration?
